...
// Hardcoded fallback data of Scandinavian and other aesthetic destinations for reliability and offline support.
...
const CURATED_DESTINATIONS: Record<string, CuratedSpot[]> = {
  stockholm: [
...
export const wayfarerHubCompileItinerary = onCall({ cors: true }, async (request) => {
  const { title, description, days, items } = (request.data ?? {}) as {
    title?: string;
    description?: string;
    days?: string[];
    items?: Array<{
